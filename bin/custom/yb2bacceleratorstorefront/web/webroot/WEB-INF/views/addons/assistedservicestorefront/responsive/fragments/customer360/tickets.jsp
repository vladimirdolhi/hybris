...
<div class="pageable-fragment asm-customer360-tab">
    <div class="asm-customer360-tickets-tab">
        ...
        <c:choose>
		 <c:when test="${ empty fragmentData.generalActivities}">
				<div class="clear">
		         	<spring:theme
        ...
        <table id="asm-customer360-tickets-table" class="table techne-table">
            ...
            <tbody>
                <c:forEach items="${fragmentData.generalActivities}" var="activityData">
                    <tr>
                        ...
                        <td data-th="Category"><spring:theme code="text.account.supporttickets.createTicket.ticketCategory.${activityData.category}" text="${activityData.category}" /></td>
                        <td data-th="Status">
                            <c:choose>
                                <c:when test="${(not empty activityData.status)}">
                                    <spring:theme code="text.customer360.activity.general.status.${activityData.status}" text="${activityData.status}" />
                                </c:when>
                                <c:otherwise>
